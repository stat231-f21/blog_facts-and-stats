...
incarceration_trends <- incarceration_trends %>%
  ...
  mutate(
    # add up prison and jail rates to create a new column
    # if one of the entries is NA, add nothing
    # sum(..., na.rm = TRUE) did not work, so use ifelse instead
    total_jail_prison_pop_rate = ifelse(is.na(total_jail_pop_rate), total_prison_pop_rate,
                                        ifelse(is.na(total_prison_pop_rate), total_jail_pop_rate,
                                               total_jail_pop_rate + total_prison_pop_rate)),
    female_jail_prison_pop_rate = ifelse(is.na(female_jail_pop_rate), female_prison_pop_rate,
                                        ifelse(is.na(female_prison_pop_rate), female_jail_pop_rate, 
                                               female_jail_pop_rate + female_prison_pop_rate)),
    male_jail_prison_pop_rate = ifelse(is.na(male_jail_pop_rate), male_prison_pop_rate,
                                        ifelse(is.na(male_prison_pop_rate), male_jail_pop_rate, 
                                               male_jail_pop_rate + male_prison_pop_rate)),
    aapi_jail_prison_pop_rate = ifelse(is.na(aapi_jail_pop_rate), aapi_prison_pop_rate,
                                        ifelse(is.na(aapi_prison_pop_rate), aapi_jail_pop_rate, 
                                               aapi_jail_pop_rate + aapi_prison_pop_rate)),
    black_jail_prison_pop_rate = ifelse(is.na(black_jail_pop_rate), black_prison_pop_rate,
                                        ifelse(is.na(black_prison_pop_rate), black_jail_pop_rate, 
                                               black_jail_pop_rate + black_prison_pop_rate)),
    latinx_jail_prison_pop_rate = ifelse(is.na(latinx_jail_pop_rate), latinx_prison_pop_rate,
                                        ifelse(is.na(latinx_prison_pop_rate), latinx_jail_pop_rate, 
                                               latinx_jail_pop_rate + latinx_prison_pop_rate)),
    native_jail_prison_pop_rate = ifelse(is.na(native_jail_pop_rate), native_prison_pop_rate,
                                        ifelse(is.na(native_prison_pop_rate), native_jail_pop_rate, 
                                               native_jail_pop_rate + native_prison_pop_rate)),
    white_jail_prison_pop_rate = ifelse(is.na(white_jail_pop_rate), white_prison_pop_rate,
                                        ifelse(is.na(white_prison_pop_rate), white_jail_pop_rate, 
...
incarceration_trends_map <- incarceration_trends_map %>% rowwise() %>%
  mutate(
    ...
    white_jail_prison_pop_rate = sum(white_prison_pop_rate, white_jail_pop_ratena.rm = TRUE)
  )

write_csv(incarceration_trends_map, "map_data/all_years_incarceration_trends_map.csv")

incarceration_trends_map <- incarceration_trends_map %>% group_by(year)
