...
incarceration_trends_1980 <- incarceration_trends %>%
  filter(year == "1980")
incarceration_trends_2018 <- incarceration_trends %>%
  filter(year == "2018")

# combine both dataframes with the us county shapefile respectively
incarceration_1980_map <- geo_join(county_shapefile, incarceration_trends_1980, 'fips', 'fips', how = "left")

leaflet(data = incarceration_1980_map) %>% 
  addTiles() %>%
...
#names(incarceration_rates ) <- c('GEOID', 'GEOID2', 'State', 'total_incarcerated', 'total_white', 'total_black','total_native', 'total_asian', 'total_pacific_islander', 'total_other', 'total_two_or_more', 'ort', 'total_white_non-hispanic', 'blank', '' )
```

```{r, eval = TRUE}

incarceration_populations <- incarceration_populations %>% rename(total_incarcerated = "...7") %>% rename( year = "Bureau of Justice Statistics")
...
incarceration_populations <- incarceration_populations  %>% mutate(total_incarcerated = as.numeric(total_incarcerated))%>% mutate(year = as.numeric(year))
```
