...
incarceration_populations <- incarceration_populations %>% rename(total_incarcerated = "...7") %>% rename( year = "Bureau of Justice Statistics") %>% rename(total_supervised = "...2")
...
incarceration_populations <- incarceration_populations  %>% mutate(total_incarcerated = as.numeric(total_incarcerated))%>% mutate(year = as.numeric(year)) %>% mutate(total_supervised = as.numeric(total_supervised))
```

```{r, eval = TRUE}
rates_by_race <- rates_by_race %>% rename(ID = State ) 
rates_by_race <- rates_by_race %>% mutate(ID = tolower(rates_by_race$ID))
rates_by_race <- rates_by_race%>% mutate(Black = as.numeric(Black))
